...
class HIRGen(
    ...
    private fun lowerDeclaration(declaration: Declaration): List<HIRDefinition> = when (declaration) {
        ...
        is Declaration.Enum -> lowerEnumDeclaration(declaration)
    }
    ...
    private fun lowerEnumDeclaration(declaration: Declaration.Enum): List<HIRDefinition> {
        ...
        val fields =
            makeList {
                add(enumStructTagName to enumTagType)
                if (declaration.cases.flatMap {it.params ?: emptyList()}.isNotEmpty() ) {
                    add(enumStructPayloadName to Type.UntaggedUnion(casePayloadTypes))
                }
            }
    ...
    private fun enumCaseDecl(
        ...
        if (case.params == null) {
            return listOf(HIRDefinition.Const(
                ...
                name = caseName.append(ctx.makeName("constructor")),
            ...
            val casePayloadType = Type.Constructor(caseName)
            return listOf(
                HIRDefinition.Struct(
                    case.location,
                    name = caseName,
                ...
                HIRDefinition.Function(
                    ...
                    HIRFunctionSignature(
                        case.name.location,
                        caseName.append(ctx.makeName("constructor")),
                        typeParams = null,
    ...
    private fun lowerMatchExpression(expression: Expression.Match): HIRExpression {
        ...
        val enumTypeDeclaration = ctx.analyzer.getEnumTypeDeclaration(expression.value.type)
        if (enumTypeDeclaration != null) {
            return lowerEnumTypeMatchExpression(expression, enumTypeDeclaration)
        }
    ...
    private fun lowerEnumTypeMatchExpression(expression: Expression.Match, declaration: Declaration.Enum): HIRExpression {
        ...
        val elseBlock = HIRBlock(
            ...
            ctx.makeUniqueName(),
            mutableListOf(
                HIRStatement.Assignment(elseBranch.location, result.name, lowerExpression(elseBranch.value))
            )
        )
        val cases = caseBranches.map { case ->
            ...
            val payloadVal = HIRStatement.ValDeclaration(case.location, ctx.makeUniqueName(), isMutable = false, casePayloadType)
            if (case.pattern.params != null) {
                statements.add(payloadVal)
                statements.add(
                    HIRStatement.Assignment(payloadVal.location, payloadVal.name, payload)
                )
            }
                    ...
                    statements.add(
                        HIRStatement.Assignment(
                            ...
                            HIRExpression.GetStructField(
                                param.location,
                                ctx.analyzer.typeOfPattern(param),
                                HIRExpression.ValRef(param.location, payloadVal.type, payloadVal.name),
                                ctx.makeName("\$$index"),
                                index
                            )
                        )
    ...
    private fun lowerPropertyExpression(expression: Expression.Property): HIRExpression = when(val binding = ctx.analyzer.resolvePropertyBinding(expression)) {
        ...
        is PropertyBinding.EnumTypeCaseConstructor -> lowerEnumCaseConstructorRef(expression, binding)
    }
    ...
    private fun lowerEnumCaseConstructorRef(expression: Expression.Property, binding: PropertyBinding.EnumTypeCaseConstructor): HIRExpression {
        ...
        return HIRExpression.GlobalRef(expression.location, expression.type, caseName)
